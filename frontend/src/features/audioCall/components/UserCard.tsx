import type { UserCardProps } from "../types/audioCall.types";

const UserCard = ({ type, isSpeaking, username, profileImage }: UserCardProps) => {
	return (
		<div className="bg-white border-2 border-slate-200/70 px-5 py-10 rounded-2xl flex flex-col items-center gap-5">
			...
			<div className="flex flex-col items-center gap-3">
				<div className="text-xl font-semibold text-slate-800 flex items-center gap-2">
					<span>{username}</span>
					{type === "self" && <span className="text-xs bg-slate-200 text-slate-600 py-0.5 px-2 rounded-md border border-slate-300">You</span>}
				</div>

...
const VideoStage = ({ remoteVideoRef, localVideoRef, localAudioRef, remoteName, localName, isLocalPinned, onSwap }: VideoStageProps) => {
	...
	const primaryParticipantStyle: CSSProperties = {
		inset: 0,
		zIndex: 0,
	};
	const previewParticipantStyle: CSSProperties = {
		zIndex: 20,
	...
	return (
		<section className="relative isolate aspect-video w-full overflow-hidden rounded-[1.25rem] bg-slate-950">
			...
			<VideoParticipant fullName={isLocalPinned ? localName : remoteName} className={primaryParticipantClassName} style={primaryParticipantStyle}>
			...
			<VideoParticipant fullName={isLocalPinned ? remoteName : localName} className={previewParticipantClassName} style={previewParticipantStyle}>
